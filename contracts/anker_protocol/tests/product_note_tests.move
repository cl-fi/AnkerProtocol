...
#[test]
fun dual_investment_note_records_product_terms() {
    let mut ctx = tx_context::dummy();
    let (registry, admin_cap) = product_note::new_registry_for_testing(1_000, @0xA, &mut ctx);
    let manager_id = object::id_from_address(@0xCAFE);
    ...
    assert_eq!(product_note::manager_id(&note), manager_id);
...
#[test]
fun shark_fin_note_records_mock_current_deposit_terms() {
    let mut ctx = tx_context::dummy();
    let (registry, admin_cap) = product_note::new_registry_for_testing(1_000, @0xA, &mut ctx);
    let manager_id = object::id_from_address(@0xCAFE);
    let oracle_id = object::id_from_address(@0xBEEF);
